...
class ExpenseRepository(
    ...
    private val merchantAliasDao: com.saikumar.expensetracker.data.dao.MerchantAliasDao? = null
) {

    /**
     * Give a merchant a user-chosen name. When the transaction has a VPA (anonymous QR
     * handle), remember the name keyed by that VPA and back-fill every other payment to the
     * same VPA - so a one-time rename sticks to all past and future payments to that shop.
     * Returns how many rows were renamed (incl. this one).
     */
    suspend fun renameMerchant(transaction: Transaction, newName: String): Int {
        val clean = newName.trim()
        if (clean.isBlank()) return 0
        var count = 0
        if (transaction.merchantName != clean) {
            transactionDao.updateTransaction(transaction.copy(merchantName = clean))
            count++
        }
        val vpa = transaction.upiId
        if (!vpa.isNullOrBlank()) {
            merchantAliasDao?.upsert(
                com.saikumar.expensetracker.data.entity.MerchantAlias(
                    rawName = vpa, canonicalName = clean, isAutoGenerated = false
                )
            )
            for (t in transactionDao.getTransactionsByUpiId(vpa)) {
                if (t.id != transaction.id && t.merchantName != clean) {
                    transactionDao.updateTransaction(t.copy(merchantName = clean))
                    count++
                }
            }
        }
     ...
     * Respects the P2P threshold (payments below it stay spending even for circle members)
     * and never touches manually-classified rows (confidenceScore >= 100).
     */
    suspend fun updateTransactionsToP2P(merchantNames: List<String>, minAmountPaise: Long = 0L) {
